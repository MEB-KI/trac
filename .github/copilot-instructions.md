...
- **Database (`database/`)**: PostgreSQL schema mapped via SQLModel and managed via Alembic migrations. Preferred flow is explicit `tud db upgrade` and optional explicit `tud studies import`; backend startup is API-only and does not perform schema/data bootstrap.
